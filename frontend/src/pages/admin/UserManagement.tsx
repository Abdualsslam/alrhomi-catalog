...
// 1. تعريف الواجهات (Interfaces) للأنواع
interface User {
  _id: string;
  username: string;
  email: string;
  role: 'rep' | 'admin' | string; // جعلناها مرنة لتقبل نصوص أخرى أو محددة
  isActive: boolean;
  createdAt: string;
  lastLogin?: string; // اختياري
  isNew?: boolean;    // خاص بالواجهة فقط (للتمييز عند الإضافة)
}

// واجهة لبيانات النموذج
interface UserForm {
  username: string;
  email: string;
  role: string;
...
// واجهة للمستخدم الذي تم إنشاؤه (يحتوي على كلمة مرور مؤقتة)
...
export default function UserManagement() {
  ...
  const [form, setForm] = useState<UserForm>({ username: '', email: '', role: 'rep' });
  ...
  const handleCreate = async () => {
    try {
      ...
      const newUser: CreatedUser = res as CreatedUser;
