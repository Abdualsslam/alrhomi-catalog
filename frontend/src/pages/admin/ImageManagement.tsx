...
interface UploadDialogProps {
  ...
  uploading: boolean;
  onClose: () => void;
  onFileChange: (key: string) => (e: ChangeEvent<HTMLInputElement>) => void;
  onUpload: () => void;
}
...
function PageHeader({ isMobile, onAdd }: PageHeaderProps) {
  return (
    ...
    >
      <Box>
        <Typography variant={isMobile ? "h5" : "h4"}>إدارة الصور</Typography>
...
function SearchToolbar({ search, onSearch, theme }: SearchToolbarProps) {
  return (
    <Paper
      elevation={0}
      sx={{
...
function EmptyState({ isMobile, search, onAdd }: EmptyStateProps) {
  return (
    <Stack
      alignItems="center"
      spacing={2}
    ...
    >
      ...
      >
        ...
        >
          تم الرفع:{" "}
          {img.uploadedAt
            ? new Date(img.uploadedAt).toLocaleDateString("ar-EG")
            : "--"}
        </Typography>
      </CardContent>
...
function UploadDialog({
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="h6">إضافة صورة جديدة</Typography>
    ...
    >
      ...
      <DialogContent
        sx={{
          ...
          justifyContent: "center",
          alignItems: "center",
          minHeight: isMobile ? "calc(100vh - 140px)" : "calc(90vh - 140px)",
        }}
      >
        <Box
          component="img"
