...
// Extended category type for tree building
...
export default function CategoryManagement() {
  ...
  const [categoriesForTree, setCategoriesForTree] = useState<Category[]>([]);
  ...
  useEffect(() => {
    setTreeLoading(true);
    API.get<PaginatedResponse<Category>>("/admin/categories", { params: { page: 1, limit: 1000 } })
      .then((res) => {
        setAllCategories(res.items);
        setCategoriesForTree(res.items);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: { name: string; description: string; parent: string | null; image?: string } = {
        name: form.name.trim(),
      ...
      setError(error.response?.data?.message || t("saveCategoryFailed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleImageFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { watermarkedUrl, originalUrl } = res || {};
      ...
      console.error(err);
      const error = err as { response?: { data?: { message?: string } } };
      setImageError(error.response?.data?.message || t("uploadImageFailed"));
    } finally {
      setImageUploading(false);
      event.target.value = "";
    }
  ...
  // مساعدة لبناء الشجرة
  function buildTree(list: Category[]): CategoryWithChildren[] {
  ...
  const renderRows = (nodes: CategoryWithChildren[], depth = 0): JSX.Element[] => {
  ...
  const openDeleteDialog = (id: string, name = "") => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      const error = err as { response?: { data?: { message?: string } } };
      setError(error.response?.data?.message || t("deleteCategoryFailed"));
    } finally {
      setDeleting(false);
    }
  };
