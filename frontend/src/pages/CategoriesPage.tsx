...
// Extended type for organized categories with children
...
export default function CategoriesPage() {
  ...
  const organizedCategories = useMemo(() => {
    ...
    const parentCategories = categories.filter((cat: Category) => !cat.parent);
    const childCategories = categories.filter((cat: Category) => cat.parent);

    // إنشاء map للفئات الفرعية حسب parent
    const childrenMap = childCategories.reduce<Record<string, Category[]>>((acc, child) => {
      const parentId =
    ...
    return parentCategories.map((parent): OrganizedCategory => ({
      ...parent,
